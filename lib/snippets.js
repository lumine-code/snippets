...
module.exports = {
    ...
    fs.stat(userSnippetsPath, (_error, stat) => {
      if (stat != null && stat.isFile()) {
        ...
        try {
          ...
          [watches]:https://lumine-code.github.io/docs.html#troubleshooting\
          `;
          lumine.notifications.addError(message, { dismissable: true });
        }
  ...
  parsedSnippetsForScopes(scopeDescriptor) {
    ...
    const legacyScopeDescriptor = lumine.config.getLegacyScopeDescriptorForNewScopeDescriptor
      ? lumine.config.getLegacyScopeDescriptorForNewScopeDescriptor(scopeDescriptor)
      : undefined;

    if (legacyScopeDescriptor) {
      unparsedLegacySnippetsByPrefix = this.selectorStore.getPropertyValue(
