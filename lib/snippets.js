...
module.exports = {
  activate(_state, { signal } = {}) {
    ...
    // The above SelectorStore will store the main registry of snippets.
    // But we need a separate SelectorStore for the snippets that come
    ...
    // Loading every snippet file can traverse the whole package fleet. Keep
    // activation synchronous so commands and the provider are available
    // immediately; callers that need the complete index can await
    // `waitForSnippetsLoaded()`.
    this.loadingPromise = Promise.resolve(this.loadAll(generation, signal));
