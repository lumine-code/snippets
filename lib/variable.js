...
function resolveClipboard({ clipboardText }) {
  return clipboardText;
}

function projectDirectoryFor(editor) {
  const filePath = editor.getPath();
  if (!filePath) return;
  // Directory containment handles project aliases and the filesystem's case
  // policy. Keep the first matching root, as the workspace variables promise.
  return lumine.project.getDirectories().find((directory) => directory.contains(filePath));
}
...
function makeDateResolver(dateParams) {
  ...
  //
  let locale = lumine.application.getLocale();
...
const RESOLVERS = {
  ...
  //
  // * Get all project directories via `lumine.project.getDirectories()`.
  ...
  RELATIVE_FILEPATH({ editor }) {
    const filePath = editor.getPath();
    return projectDirectoryFor(editor)?.relativize(filePath) ?? filePath ?? "";
  },

  // Last path component of the ancestor project path.
  WORKSPACE_NAME({ editor }) {
    const directory = projectDirectoryFor(editor);
    return directory ? path.basename(directory.getPath()) : "";
  },

  // The full path to the ancestor project path.
  WORKSPACE_FOLDER({ editor }) {
    return projectDirectoryFor(editor)?.getPath() ?? "";
  },
