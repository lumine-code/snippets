...
let SelectorStore = null;
let Snippet = null;
let SnippetExpansion = null;
let EditorStore = null;

const getSelectorStore = () => (SelectorStore ||= require("@lumine-code/selector-store"));
const getSnippet = () => (Snippet ||= require("./snippet"));
const getSnippetExpansion = () => (SnippetExpansion ||= require("./snippet-expansion"));
const getEditorStore = () => (EditorStore ||= require("./editor-store"));
...
function showCommandNameConflictNotification(name, commandName, packageName, snippetsPath) {
  ...
  lumine.notifications.addError(`Snippets conflict`, {
    description: message,
    dismissable: true,
  });
}
...
function showInvalidCommandNameNotification(name, commandName) {
  ...
  lumine.notifications.addError(`Snippets error`, {
    description: message,
    dismissable: true,
  });
}

// When we first run, checking `lumine.commands.registeredCommands` is a good way
...
const CommandMonitor = {
  ...
  exists(commandName) {
    ...
    if (!map.has(commandName)) {
      ...
      let value = lumine.commands.registeredCommands[commandName];
  ...
  remove(commandName) {
    ...
    if (disposable) {
      disposable.dispose();
      this.disposables.delete(commandName);
    }
  },
...
const PackageNameResolver = {
  ...
  setup() {
    ...
    let meta = lumine.packages.getLoadedPackages() || [];
    for (let { name, path } of meta) {
      this.pathsToNames.set(path, name);
    }
    if (!this.subscriptions) {
      this.subscriptions = new CompositeDisposable(
        lumine.packages.onDidLoadPackage(() => this.setup()),
        lumine.packages.onDidUnloadPackage(() => this.setup()),
      );
    }
  },
  reset() {
...
module.exports = {
  provideBackgroundTips() {
    return {
      ...
      tips: [
        "You can expand a snippet by typing its prefix and pressing {{ 'snippets:expand' | keystroke }}",
      ],
    };
  },

  ensureStores() {
    if (this.selectorStore && this.disabledSnippetsSelectorStore) return;
    const Store = getSelectorStore();
    this.selectorStore = new Store();
    // Disabled-package snippets stay out of active lookup while remaining
    // available to settings-view's inventory.
    this.disabledSnippetsSelectorStore = new Store();
  },
  ...
  activate(_state, { signal } = {}) {
    ...
    this.handlingUserSnippetsGeneration = null;

    this.selectorStore = null;
    this.disabledSnippetsSelectorStore = null;

    this.subscriptions = new CompositeDisposable();
    this.subscriptions.add(
      lumine.workspace.addOpener((uri) => {
        if (uri === "lumine://.lumine/snippets") {
          return lumine.workspace.openTextFile(this.getUserSnippetsPath());
        }
      }),
    );

    // Selector stores and the package-wide file walk are cold-start work.
    // Publish commands/provider synchronously, then start the index and file
    // watcher after the activation batch leaves the stack.
    let startLoading;
    this.loadingPromise = new Promise((resolve, reject) => {
      startLoading = () => {
        if (this.activationGeneration !== generation || !this.subscriptions) {
          resolve(false);
          return;
        }
        try {
          ...
          Promise.resolve(this.loadAll(generation, signal)).then(resolve, reject);
        } catch (error) {
          reject(error);
        }
      };
    ...
    queueMicrotask(() => {
      if (this.activationGeneration === generation && this.subscriptions) startLoading();
    });

    this.subscriptions.add(
      lumine.config.onDidChange("core.packagesWithSnippetsDisabled", ({ newValue, oldValue }) => {
    ...
    // Expansion and the tab stops stay on the editor element. Their contract is
    // aborting the key binding so `tab` falls through to the next one, which a
    // dispatch from the application menu cannot use — so none of them is in
    // one. The element scope is also what makes them work in an editor that is
    // not a pane item, which a workspace registration could never reach.
    this.subscriptions.add(
      lumine.commands.add("lumine-text-editor", {
        ...
        "snippets:next-tab-stop": {
          description: "Move to the next placeholder of the expanded snippet.",
          didDispatch(event) {
            const editor = this.getModel();
        ...
        "snippets:previous-tab-stop": {
          description: "Move back to the previous placeholder of the snippet.",
          didDispatch(event) {
            const editor = this.getModel();
    ...
    this.subscriptions.add(
      lumine.commands.add("lumine-workspace", {
        "snippets:available": {
          description: "List the snippets that apply to this file's grammar.",
          didDispatch() {
  ...
  deactivate() {
    ...
    this.availableSnippetsView?.destroy();
    this.availableSnippetsView = null;
    lumine.config.transact(() => this.subscriptions?.dispose());
    this.subscriptions = null;
    CommandMonitor.reset();
    PackageNameResolver.reset();
  },
  ...
  getUserSnippetsPath() {
    ...
    this.userSnippetsPath = CSON.resolve(path.join(lumine.getConfigDirPath(), "snippets"));
    if (this.userSnippetsPath == null) {
      this.userSnippetsPath = path.join(lumine.getConfigDirPath(), "snippets.json");
    }
    return this.userSnippetsPath;
  },

  loadAll(generation = this.activationGeneration, signal) {
    this.ensureStores();
    return new Promise((resolve) => {
      ...
      const isCurrent = () => generation === this.activationGeneration && !signal?.aborted;
      const aborted = () => {
        if (generation === this.activationGeneration) this.activationGeneration++;
        finish(false);
      };
  ...
  loadBundledSnippets(callback, generation = this.activationGeneration) {
  ...
  loadUserSnippets(callback, generation = this.activationGeneration) {
    const userSnippetsPath = this.getUserSnippetsPath();
    fs.stat(userSnippetsPath, (error, stat) => {
      if (generation !== this.activationGeneration) return callback({});
  ...
  watchUserSnippets(callback) {
    ...
    const file = watchFile(userSnippetsPath);
    const reload = () => {
      if (!subscriptions.disposed) this.handleUserSnippetsDidChange();
    };
    subscriptions.add(
      file,
      file.onDidChange(reload),
      file.onDidInvalidate(reload),
      file.onDidError((error) => {
        lumine.notifications.addError(`Unable to watch ${path.basename(userSnippetsPath)}`, {
  ...
  handleUserSnippetsDidChange() {
    ...
    if (this.handlingUserSnippetsGeneration === generation) {
      return;
    }

    this.handlingUserSnippetsGeneration = generation;
    lumine.config.transact(() => {
  ...
  removeSnippetsForPackage(packageName) {
    this.ensureStores();
    const snippetSet = this.snippetsByPackage.get(packageName);
    // Copy these snippets to the "quarantined" SelectorStore so that they
  ...
  loadPackageSnippets(callback, generation = this.activationGeneration) {
    const disabledPackageNames = lumine.config.get("core.packagesWithSnippetsDisabled") || [];
    const packages = lumine.packages.getLoadedPackages().sort((pack, _) => {
    ...
    async.map(
      snippetsDirPaths,
      (snippetsDirPath, done) => this.loadSnippetsDirectory(snippetsDirPath, done, generation),
      (error, results) => {
        if (generation !== this.activationGeneration) return callback({});
        const zipped = [];
        ...
        for (const o of zipped) {
          ...
          if (disabledPackageNames.includes(o.pack.name)) {
            ...
            // SelectorStore, we'll keep track of them in a separate
            // SelectorStore so that they can still be represented in the
            // settings view.
  ...
  waitForSnippetsLoaded() {
    ...
    return this.loadingPromise || Promise.resolve(false);
  },

  doneLoading(generation = this.activationGeneration) {
    if (generation !== this.activationGeneration) return;
    this.loaded = true;
  ...
  loadSnippetsDirectory(snippetsDirPath, callback, generation = this.activationGeneration) {
    fs.stat(snippetsDirPath, (error, stat) => {
      if (generation !== this.activationGeneration) return callback(null, {});
      ...
      fs.readdir(snippetsDirPath, (error, entries) => {
        ...
        async.map(
          ...
          (entry, done) => {
            ...
            this.loadSnippetsFile(
              filePath,
              (snippets) => done(null, { filePath, snippets }),
              generation,
            );
          },
          (error, results) => {
  ...
  loadSnippetsFile(filePath, callback, generation = this.activationGeneration) {
    if (!CSON.isObjectPath(filePath)) {
      return callback({});
    }
    CSON.readFile(filePath, { allowDuplicateKeys: false }, (error, object = {}) => {
      if (generation !== this.activationGeneration) return callback({});
      if (error?.code === "ENOENT" && filePath === this.getUserSnippetsPath()) {
        // A first-run or deleted user file is an empty snippet set. The fixed
        // path remains watched so a later creation is loaded normally.
        return callback({});
      }
      if (error != null) {
        console.warn(
        ...
        lumine.notifications.addError(`Failed to load snippets from '${filePath}'`, {
          detail: error.message,
          dismissable: true,
        });
      }
  ...
  addCommandForSnippet(attributes, packageName, _selector) {
    ...
    let commandHandler = (event) => {
      let editor = event.target.closest("lumine-text-editor").getModel();
      ...
      return this.expandSnippet(editor, targetSnippet);
    };

    let disposable = lumine.commands.add("lumine-text-editor", commandName, commandHandler);
  ...
  storeUnparsedSnippets(value, path, selector, packageName, isDisabled = false) {
    ...
    const store = isDisabled ? this.disabledSnippetsSelectorStore : this.selectorStore;
    store.addProperties(path, unparsedSnippets, { priority: this.priorityForSource(path) });
  },

  clearSnippetsForPath(path) {
    this.ensureStores();
    for (const scopeSelector in this.selectorStore.propertiesForSource(path)) {
      let object = this.selectorStore.propertiesForSourceAndSelector(path, scopeSelector);
      if (object.snippets) {
      ...
      this.selectorStore.removePropertiesForSourceAndSelector(path, scopeSelector);
    }
  },

  parsedSnippetsForScopes(scopeDescriptor) {
    this.ensureStores();
    const unparsedSnippetsByPrefix = this.selectorStore.getPropertyValue(
  ...
  clipboardTextForSnippet(snippet) {
    return snippet.variables.some((variable) => variable.name === "CLIPBOARD")
    ...
    const expand = (clipboardText) => {
      ...
      editor.transact(() => {
        ...
        for (const cursor of cursors) {
          this.insert(snippet, editor, cursor, { method: "command", clipboardText });
        }
      });
    };

    const clipboardText = this.clipboardTextForSnippet(snippet);
    return clipboardText != null && typeof clipboardText.then === "function"
      ? clipboardText.then(expand)
      : expand(clipboardText);
  },
  ...
  expandSnippetsUnderCursors(editor) {
    ...
    const expand = (clipboardText) => {
      // The clipboard read crosses to the main process. If the cursor moved in
      // that brief interval, do not expand a prefix that is no longer there.
      if (this.snippetToExpandUnderCursor(editor) !== snippet) return false;
      ...
      editor.transact(() => {
        ...
        for (const cursor of cursors) {
          ...
          this.insert(snippet, editor, cursor, { method: "prefix", clipboardText });
        }
      });
      return true;
    };
  ...
  getStore(editor) {
    return getEditorStore().findOrCreate(editor);
  },
  ...
  async insert(
    ...
    editor,
    cursor,
    { method = null, adjustIndentation = true, clipboardText } = {},
  ) {
    if (editor == null) {
    ...
    const createExpansion = (resolvedClipboardText) =>
      new (getSnippetExpansion())(snippet, editor, cursor, this, {
        method,
        adjustIndentation,
        clipboardText: resolvedClipboardText,
      });
    if (clipboardText === undefined) {
      const didRead = this.clipboardTextForSnippet(snippet);
      clipboardText =
        didRead != null && typeof didRead.then === "function" ? await didRead : didRead;
    }
    return createExpansion(clipboardText);
  },
  ...
  getUnparsedSnippets() {
    ...
    iterate(this.selectorStore.propertySets);
    iterate(this.disabledSnippetsSelectorStore.propertySets);
    return results;
  },
